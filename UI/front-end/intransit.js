...
const fetchData = async (token, user) => {
  fetch(`/api/v1/users/${user.id}/parcels?status=${STATUS_INTRANSIT}`, {
    method: "GET",
    headers: {
    ...
    .then(response => {
      response.json().then(result => {
        ...
        parcels.map(parcel => {
          ...
          <tr class="clickable" onClick=getParcel(${parcel.id})>
              <td>${parcel.pickup_location}</td>
              <td>${parcel.address.pickup_address || ""}</td>
              <td class="editable">
                  <span>${parcel.destination}</span>
              </td>
              <td>${parcel.address.destination_address || ""}</td>
              <td>
                  <span> Q:${parcel.details.quantity}</span>
                  <span> W:${parcel.details.weight}</span>
                 ${
                   parcel.details.length
                     ? `<span> L:${parcel.details.length}</span>`
                     : ""
                 }
                  ${
                    parcel.details.height
                      ? `<span> H:${parcel.details.height}</span>`
                      : ""
                  }
              </td>
              <td>${new Date(parcel.created_at).toDateString()}</td>
              <td class="status">
                  <span class=${parcel.status}>in transit</span>
                  <span class="icon-cancel" onClick=onCancel(${parcel.id})>
                  </span>
              </td>
          </tr>
        `);
